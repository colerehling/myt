...
db.query(`
  CREATE TABLE IF NOT EXISTS squares (
    ...
    y INTEGER,
    owner VARCHAR(255),
    entry_id INTEGER REFERENCES map_entries(id),
    UNIQUE(x, y)
  )
`);
...
app.post("/api/entries", async (req, res) => {
  ...
  try {
    ...
    const x = Math.floor(lng * 20000);
    const y = Math.floor(lat * 20000);

    await db.query(`
      INSERT INTO squares (x, y, owner, entry_id)
      VALUES ($1, $2, $3, $4)
      ON CONFLICT (x, y) DO UPDATE
      SET owner = $3, entry_id = $4
    `, [x, y, username, entryId]);
...
app.get("/api/squares", async (req, res) => {
  try {
    const result = await db.query(`
      SELECT s.x, s.y, s.owner, m.latitude, m.longitude, m.text
      FROM squares s
      JOIN map_entries m ON s.entry_id = m.id
    `);
